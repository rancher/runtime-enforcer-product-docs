...
[arabic]
. Clone the repository: `git clone https://github.com/rancher-sandbox/runtime-enforcer.git`.
. Install https://kubernetes.io/docs/reference/kubectl/[kubectl] and https://helm.sh/[helm].
. Install https://docs.tilt.dev/install.html[tilt].
. Install https://github.com/libbpf/libbpf[libbpf], so you can build ebpf programs at your local.
...
[arabic]
. Install golangci-lint, e.g., `go install github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.6.0`
...
If you are using https://pre-commit.com/[pre-commit], you can run the following command to install the hooks we define in `.pre-commit-config.yaml`:
